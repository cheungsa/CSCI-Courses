 ...
 * Date      : 4/11/17
 * Purpose   : Reads an image and identifies different objects in the image 
               and labels them, assigning a different color to each 
               identified object
 * Notes     : Uses the method connected-component labeling to identify the 
               different objects
 *************************************************************************/
...
int component_labeling(unsigned char **binary_image,int **labeled_image,int  
   height,int width);
...
//This function is used to test your queue implementation. You do not need 
//to change it, though doing so might be useful/helpful
...
// takes pointer to 3D array, containing image
// places a grey-scale version of the image to 
void rgb2gray(unsigned char ***in,unsigned char **out,int height,int width) 
{
   for (int i=0; i<height; i++)
   {
         ...
         if (label[i][j] == 0 && binary_image[i][j] == 1)
         {
            ...
            while (!q.is_empty())
            {
               ...
               binary_image[white.row][white.col] == 1)
               {
                  ...
                  // Check surroundings arround pixel, 4 directions
                  // North
                  if (white.row-1 > 0 && label[white.row-1][white.col] == 0)
                  {
                     // should have just incremented but too lazy 
                     white.row -= 1; 
                     q.push(white); // push prev, move on to next
                     white.row += 1;
                  }
                  // South
                  if (white.row+1 < height && label[white.row+1]
                  [white.col] == 0)
                  {
                     white.row += 1;
                     q.push(white);
                  ...
                  // West
                  if (white.col-1 > 0 && label[white.row][white.col-1] == 0)
                  {
                     white.col -= 1;
                     q.push(white);
                     white.col += 1;
                  ...
                  // East
                  if (white.col+1 < height && label[white.row]
                  [white.col+1] == 0)
                  {
                     white.col += 1;
                     q.push(white);
...
//First make num_segments number of random colors to use for coloring the 
//labeled parts of the image.
//Then loop over the labeled image using the label to index into your random 
//colors array.
//Set the rgb_pixel to the corresponding color, or set to black if the pixel 
//was unlabeled.
#ifndef AUTOTEST
void label2RGB(int  **labeled_image, unsigned char ***rgb_image,int 
num_segments,int height,int width)
{ 
   // dynamic mem allocation for color assignment
   int** colored = new int* [num_segments]; 
   for (int i=0; i<num_segments; i++)
   {
      colored[i] = new int[3]; // RGB = 3
   }
   ...
   for (int i=0; i<num_segments; i++)
   {
      for (int j=0; j<3; j++)
      {
         colored[i][j] = rand() % 256; // randomize for SIZE = 256
      }
   }
   ...
   for (int i=0; i<height; i++)
   {
      for (int j=0; j<width; j++)
      {
         for (int l=0; l<num_segments; l++)         
         {
            for (int k=0; k<3; k++)
            {
               if (labeled_image[i][j] == l) // color if one of seg
               {
...
void clean(unsigned char ***input,unsigned char **gray,unsigned char 
   **binary, int **labeled_image,int height , int width) {
    if(input ) {
